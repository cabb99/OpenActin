...
def test_from_pdb():
    s = coarseactin.Scene.from_pdb(f'coarseactin/tests/data/1zir.pdb')
    assert len(s) == 1771
    atom = s.loc[1576]
    #print(atom)
...
def test_from_cif():
    s = coarseactin.Scene.from_cif(f'coarseactin/tests/data/1zir.cif')
    assert len(s) == 1771
    atom = s.loc[1576]
    #print(atom)
    assert atom['serial'] == 1577
...
class Test_Read_Write():
    def _convert(self, reader, writer, mol):
        if reader == 'pdb':
            s1 = coarseactin.Scene.from_pdb(f'coarseactin/tests/data/{mol}.pdb')
        elif reader == 'cif':
            s1 = coarseactin.Scene.from_cif(f'coarseactin/tests/data/{mol}.cif')
        elif reader == 'gro':
            s1 = coarseactin.Scene.from_gro(f'coarseactin/tests/data/{mol}.gro')
        elif reader == 'fixPDB_pdb':
            s1 = coarseactin.Scene.from_fixPDB(pdbfile=f'coarseactin/tests/data/{mol}.pdb')
        elif reader == 'fixPDB_cif':
            s1 = coarseactin.Scene.from_fixPDB(pdbxfile=f'coarseactin/tests/data/{mol}.cif')
