...
#                  "actinLen": [13,100],
    ...
    if len(sys.argv) > 1:
        try:
            ...
            pass
    #sjob = openactin.SlurmJobArray("Simulations_nots/SingleABP/SingleABP", parameters, test_parameters, job_id)
    sjob = openactin.SlurmJobArray("Simulations/DoubleABP/DoubleABP", parameters, test_parameters, job_id)
    ...
    for i in range(n):
        ...
        model += [s]
    model = openactin.Scene.concatenate(model)
    model = openactin.Scene(model[(model['resName'] != abp) | model['chainID'].isin(['B','C'])])

    #Split and rejoin the model so that ABPs are on a different chain
    full_model = openactin.Scene(model.sort_values(['chainID', 'resSeq', 'name']))
