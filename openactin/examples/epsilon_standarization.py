...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        try:
            ...
            pass
    # sjob = openactin.SlurmJobArray("Simulations_nots/Box/Box", parameters, test_parameters, job_id)
    sjob = openactin.SlurmJobArray("Simulations/Box/Epsilon_Box3uM", parameters, test_parameters, job_id)
    ...
    for i in range(sjob["n_abps"] // 2):
        full_model += [openactin.create_abp(translation=np.random.random(3) * sjob["box_size"],
                                              rotation=strans.Rotation.random().as_matrix(),
                                              abp=sjob['abp'])]
    unbound_actins = openactin.Scene.concatenate(full_model)
    ...
    for i in range(sjob["n_actins"] - sjob["n_actins"] // 2):
        full_model += [openactin.create_actin(length=sjob["actinLen"],
                                                ...
                                                rotation=strans.Rotation.random().as_matrix(),
                                                abp=sjob['abp'])]
    bound_actins = openactin.Scene.concatenate(full_model)
    ...
    full_model = openactin.Scene.concatenate([unbound_actins, bound_actins])
